...
- **Projects** — Auto-generated project codes (workspace-configurable prefix, e.g. WRI-001, WRI-002, …), project types with color tags, budgeted hours with usage progress bar, file attachments, and team member allocation (with a designated SPOC per project); assigned members are emailed when added to a project. An All / Active / Completed / On Hold / Cancelled status tab bar (each with a total-count badge) sits above the list. The list view is kept compact (Code, Project Name, Type, Request Date, Client, Users Assigned, Status, TAT) and sorts by Request Date (newest first) by default; Requestor and Credits are omitted from the list and shown only in the detail panel, which surfaces every project field (Request Date, Requestor, Report Initiated, Report Delivered, TAT, Users Assigned, Credits, budget usage, members, timesheets, and documents). A "Users Assigned" column/field shows project members as soon as they're assigned, regardless of whether they've logged time yet, and is visible to all roles, not just admins. Admins can export the full project list to CSV.
...
- **Reports** — Time by user, project, client, or period (Daily / Weekly / Monthly, with business-day TAT per project), with budget vs. logged hours visualization, CSV export, and a report delivery calendar view showing delivered/upcoming/overdue report dates (cancelled projects are shown in their own status rather than as overdue); "Users Assigned" reflects project membership, not just users who've logged time. The By Project view has an All / Active / Completed / On Hold / Cancelled status filter
- **Daily backup email** — Every day at 8:00 AM IST, the server emails a workbook (Projects + Timesheet Entries sheets) to a fixed recipient; sent via Microsoft Graph `sendMail` (trying each configured `SUPER_ADMIN_EMAIL` sender in turn) when Microsoft SSO is configured, otherwise via SMTP. A super admin can also trigger it on demand from `POST /api/admin/send-backup-now`
...
**Production deployment (current):** the GitHub Actions workflow (`.github/workflows/deploy.yml`) runs both on `workflow_dispatch` (Actions tab → Deploy to Production → Run workflow, on `main`) and automatically on every push to `main` — the `push: branches: [main]` trigger was disabled after the incident above, then deliberately re-enabled once the manual-trigger path had run stably for a while (see the "Enable auto-deploy on push to main" commit). Pushing to `main` now deploys by itself; a manual dispatch is only needed to re-run a deploy without a new commit. Once triggered (by either path), it rsyncs the repo to `/var/www/time-tracking/` on the server, runs `docker compose up -d --build --remove-orphans`, and polls `docker inspect`'s health status until the container reports `healthy` (failing the workflow and dumping the last 100 log lines if it doesn't within ~2 minutes).
...
├── .github/workflows/deploy.yml  # Deploys to Docker on the production host — runs on workflow_dispatch and on every push to main, see Production Build & Deployment
├── .env.example
└── package.json
```
