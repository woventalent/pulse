...
**Production deployment (currently manual — see warning below):** the GitHub Actions workflow (`.github/workflows/deploy.yml`) rsyncs the repo to `/var/www/time-tracking/` on the server, runs `docker compose up -d --build --remove-orphans`, and then polls `docker inspect`'s health status until the container reports `healthy` (failing the workflow and dumping the last 100 log lines if it doesn't within ~2 minutes). It normally runs automatically on every push to `main`, but **that trigger is temporarily disabled** — see the note directly below.

> **⚠️ Deploy trigger currently disabled.** `deploy.yml`'s `push: branches: [main]` trigger is commented out. It turns out `/var/www/time-tracking/` (this workflow's rsync target) is **not** the real production checkout — production actually runs from a separate, independently-managed path at `/opt/alternative-ens-deployment/time-tracking/`. Until the two are reconciled (either repoint this workflow at the real path, or retire the other deployment), **pushing to `main` does not deploy anything**; use `workflow_dispatch` only if you've confirmed which path is authoritative. See the comment at the top of `deploy.yml` for the full context.
